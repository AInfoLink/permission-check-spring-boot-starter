package com.module.app.security.web.filter


import com.module.app.security.service.UserJwtService
...
class UserJwtAuthenticationFilter(
    ...
    override fun doFilterInternal(
        ...
        cookies?.forEach { cookie: Cookie ->
            if (cookie.name == userJwtService.IDENTITY_KEY) token = cookie.value
        }
        ...
        if (!userJwtService.isValidToken(token)) {
        ...
        val userDetails = userJwtService.parseToken(token)
        if (userDetails == null) {
            response.status = HttpServletResponse.SC_UNAUTHORIZED
            return
        }
